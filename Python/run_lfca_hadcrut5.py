...
tas = tas0.transpose(2, 1, 0)
lat_axis =np.array(filein.variables['latitude'])
lon_axis =np.array(filein.variables['longitude'])
nlon = tas.shape[0]
nlat =tas.shape[1]
ntime = tas.shape[2]
time = np.arange(1850,2024+8.1/12,1/12.)

cutoff = 120
truncation = 30
#%%
st_month = (1957-1850)*12

mean_seasonal_cycle = np.zeros((nlon,nlat,12))
tas_anomalies0 = np.zeros((nlon,nlat,ntime))
for i in range(12):
    mean_seasonal_cycle[...,i] = np.nanmean(tas[...,i+st_month:ntime:12],-1)
    tas_anomalies0[...,i:ntime:12] = tas[...,i:ntime:12] - mean_seasonal_cycle[...,i][...,np.newaxis]
#%%


dir_path = os.path.dirname(os.path.realpath(__file__))
load_file = dir_path+"/../ERSST_firstHadCRUT5_s1975.npy"
monthly_full = np.full((123-70, ntime-st_month ), np.nan)
for i in range(75,125):
    print(i)
    thisrun_row = i-70
    thisrun_ntime= (50+i)*12
    tas_anomalies = tas_anomalies0[...,st_month:thisrun_ntime] #if we restrict to only 10 years - get el nino as major signal, so start with 40
    s = tas_anomalies.shape
    y, x = np.meshgrid(lat_axis,lon_axis)
    area = np.cos(y*np.pi/180.)
    area[np.where((np.abs(np.mean(tas_anomalies,-1))>1e20))] = 0
    print(np.sum((np.abs(np.mean(tas_anomalies,-1))<1e20)) / area.shape[0] / area.shape[1])
    #%%
    domain = np.ones(area.shape)
    #%%
    order = 'C'
    x = np.transpose(np.reshape(tas_anomalies,(s[0]*s[1],s[2]),order=order))
   ...
   # pattern[np.where(np.abs(pattern)>1.e5)] = np.nan
    monthly_full[thisrun_row, 0: (-st_month+thisrun_ntime)] = np.real(lfcs[:,imode])
